  ...
  object SPMessageSyntax {
    case class SPMessageExtractor[H, B](data: Option[(H, B)]) {
      ...
      def flatMapBody[B2](f: PartialFunction[B, Option[B2]]): SPMessageExtractor[H, B2] = {
        copy(data = data.flatMap { case (h, b) => f.applyOrElse(b, _ => None).map(b2 => (h, b2)) })
      }
      def flatMapHeader[H2](f: B => Option[H2]): SPMessageExtractor[H2, B] = {
      ...
      def map[C](op: (H, B) => C): Option[C] = data.map(op(_))
      def foreach(op: (H, B) => Unit): Unit = data.foreach(op(_))
